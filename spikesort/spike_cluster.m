function clusters=spike_cluster(features)
    NUM_CLUSTERS = 3;
    classes = kmeans(features', NUM_CLUSTERS, 'MaxIter', 300);

% uncomment for visualizatin     
%     
%     ColorSet = [
...
%         class = features(:,find(classes == i));
%         scatter3(class(1,:), class(2,:), class(3,:),3, ColorSet(i,:));
%     end
    clusters = classes;
end